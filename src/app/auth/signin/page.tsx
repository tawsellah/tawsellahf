...
import { signInWithEmailAndPassword, sendPasswordResetEmail, GoogleAuthProvider, signInWithRedirect, getRedirectResult, type User } from 'firebase/auth';
...
export default function SignInPage() {
      ...
      try {
        ...
        if (result && result.user) {
          ...
          if (!snapshot.exists()) {
            ...
            toast({ title: "أهلاً بك مجدداً!", description: "تم تسجيل دخولك بنجاح باستخدام Google." });
          }
          router.push('/');
        }
      } catch (error: any) {
        ...
        if (error.code !== 'auth/web-storage-unsupported') {
            ...
            description: error.message || "فشل تسجيل الدخول باستخدام Google. الرجاء المحاولة مرة أخرى.",
            variant: "destructive",
            });
        }
      }
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      toast({
        title: "خطأ في بدء تسجيل الدخول",
        description: error.message || "فشل بدء عملية تسجيل الدخول باستخدام Google.",
        variant: "destructive",
      });
    }
  };
